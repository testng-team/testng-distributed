...
public abstract class DistributedTest extends BaseDistributedTest {
  ...
  throws IOException
  {
    ...
    p.setProperty(DefaultMastertAdapter.HOSTS, "localhost:" + m_ports[0] + " localhost:" + m_ports[1]);
    p.setProperty(SuiteDispatcher.MASTER_STRATEGY, strategy);
    p.setProperty(SuiteDispatcher.VERBOSE, "0");
    p.setProperty(SuiteDispatcher.MASTER_ADAPTER, DefaultMastertAdapter.class.getName());
  ...
  throws IOException
  {
    ...
    result.deleteOnExit();
    Properties p = new Properties();
    p.setProperty(SuiteSlave.VERBOSE, "0");
    p.setProperty(DefaultWorkerAdapter.SLAVE_PORT, port);
    p.setProperty(SuiteSlave.SLAVE_ADPATER, DefaultWorkerAdapter.class.getName());
    FileOutputStream fos = new FileOutputStream(result);
    p.store(fos, "Automatically generated by tests");
    fos.close();
